...
"your-hostname" = mkNixos {
  ...
  homeModules = [ ./home/linux/wayland.nix ];  # or x11.nix for X11/i3
};
```
...
# GUI (WSL with WSLg, Wayland desktop)
...
│   └── thinkpad-x200s/          # imports nixos/common + nixos/bios + nixos/x11
│
...
`hosts/<hostname>/hardware-configuration.nix` is generated by the NixOS installer and is not tracked in this repo. Run `nixos-generate-config` during installation, copy the result to `hosts/<hostname>/`, then run `install.sh`.
...
For NixOS:
```
modules/nixos/common.nix         ← networking, users, zsh, flakes, linger (all NixOS)
modules/nixos/uefi.nix           ← UEFI bootloader (systemd-boot)
modules/nixos/bios.nix           ← BIOS bootloader (grub)
modules/nixos/x11.nix            ← X11 + i3 (xserver, startx, windowManager.i3)
